...
from robo_manip_baselines.common.models import AnyTouchTactileEncoder


class DiffusionPolicyWithAnyTouch(BaseImagePolicy):
    def __init__(
        self,
        shape_meta: dict,
        noise_scheduler: DDPMScheduler,
        anytouch_checkpoint_path,
        anytouch_config,
        horizon,
        n_action_steps,
        n_obs_steps,
        ...
        **kwargs,
        ...
        # create obs encoder
        obs_feature_dim = 64
        obs_encoder = AnyTouchTactileEncoder(
            shape_meta["obs"],
            anytouch_checkpoint_path,
            anytouch_config,
            tactile_pooling=tactile_pooling,
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_global_cond:
            # reshape B, T, ... to B*T
            def reshape_items(item):
                if len(item.shape) >= 3:
                    return item[:, : self.n_obs_steps, ...].reshape(-1, *item.shape[2:])
                elif not isinstance(item, torch.Tensor) or len(item.shape) < 3:
                    return item
                else:
                    return item[:, : self.n_obs_steps].reshape(-1)

            this_nobs = dict_apply(nobs, reshape_items)
            nobs_features = self.obs_encoder(this_nobs)
            # reshape back to B, Do
