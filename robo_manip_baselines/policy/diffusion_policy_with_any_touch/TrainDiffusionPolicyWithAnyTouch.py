...
class TrainDiffusionPolicyWithAnyTouch(TrainBase):
    DatasetClass = DiffusionPolicyWithAnyTouchDataset
    ...
    def set_additional_args(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="path of trained AnyTouch checkpoint",
        )
        parser.add_argument(
            "--anytouch_config",
            type=str,
            default="./common/models/configs/AnyTouchConfig.json",
            help="path of config json for AnyTouch",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            default=["gelsight_left", "gelsight_right"],
            help="Camera names of RGB tactile sensors",
        )
        parser.add_argument(
            "--tactile_pooling",
            type=str,
            choices=["cls", "global"],
            default="cls",
            help="Pooling method for tactile embedding",
        )
        parser.add_argument(
            "--use_universal_sensor_token",
            action="store_true",
            help="Use universal sensor token instead of specific sensor token",
        )
        parser.add_argument(
            "--use_sensor_token",
            action="store_true",
            help="Use sensor token for tactile embedding",
        )
        parser.add_argument(
            "--use_same_patchemb",
            action="store_true",
            help="Use same patchemb in AnyTouch or not",
        )
    ...
    def setup_model_meta_info(self):
        ...
        self.model_meta_info["data"]["tactile_camera_names"] = (
            self.args.tactile_camera_names
        )
        ...
        self.model_meta_info["data"]["use_universal_sensor_token"] = (
            self.args.use_universal_sensor_token
        )
        ...
        self.model_meta_info["policy"]["anytouch_checkpoint"] = (
            self.args.anytouch_checkpoint
        )
        self.model_meta_info["policy"]["anytouch_config"] = CLIPConfig.from_json_file(
            self.args.anytouch_config
        )
    ...
    def setup_policy(self):
        # Set AnyTouch config
        anytouch_config = self.model_meta_info["policy"]["anytouch_config"]
        ...
        for camera_name in self.args.camera_names:
            shape_meta["obs"]["image"][DataKey.get_rgb_image_key(camera_name)] = {
                "shape": [3, self.args.image_size[1], self.args.image_size[0]],
                "type": "rgb",
            }
        for camera_name in self.args.tactile_camera_names:
            shape_meta["obs"]["tactile_image"][
                DataKey.get_rgb_image_key(camera_name)
            ] = {
                "shape": [3, 224, 224],
                "type": "rgb",
            }
            shape_meta["obs"]["tactile_sensor_type"][camera_name] = {
                "shape": [1],
                "type": "low_dim",
            }
        self.model_meta_info["policy"]["args"] = {
            ...
            "anytouch_checkpoint_path": self.args.anytouch_checkpoint,
            "anytouch_config": anytouch_config,
            "use_sensor_token": self.args.use_sensor_token,
            "use_same_patchemb": self.args.use_same_patchemb,
            "tactile_pooling": self.args.tactile_pooling,
        }
        if self.args.backbone == "cnn":
        ...
        print(f"  - tactile names: {self.args.tactile_camera_names}")
