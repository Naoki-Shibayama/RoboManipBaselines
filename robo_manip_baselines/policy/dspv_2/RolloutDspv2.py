...
class RolloutDspv2(RolloutBase):
    ...
    def setup_policy(self):
        ...
        print(
            f"  - views: {self.model_meta_info['image']['view_camera_names']}, action steps: {self.model_meta_info['data']['n_action_steps']}"
        )
    ...
    def infer_policy(self):
        ...
        if self.policy_action_buf is None or len(self.policy_action_buf) == 0:
            ...
            if len(self.view_camera_names) > 0:
                v_images = self.get_images()
            pointcloud = self.get_pointcloud()
            action = self.policy(pointcloud, qpos=state, imgs=v_images)[0]
    ...
    def get_pointcloud(self):
        ...
        pcd_feats_tensor = torch.tensor(pcd_feats, dtype=torch.float32).to(self.device)
        pcd_bcoords, pcd_bfeats = ME.utils.sparse_collate(
            [pcd_coords_tensor], [pcd_feats_tensor], dtype=torch.float32
        )
        pointcloud_sparse = ME.SparseTensor(pcd_bfeats, pcd_bcoords)
        return pointcloud_sparse

    def get_images(self):
        images = []
        for camera_name in self.view_camera_names:
        ...
        return torch.stack(images, dim=0)[torch.newaxis].to(self.device)
